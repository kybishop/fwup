...
#error Fill in with an example SDCard/MMC device.
    ...
    printf("  --block-cache-size <MB> Block cache size in MB (8-1024, default 8)\n");
...
int main(int argc, char **argv)
{
    ...
    size_t block_cache_size_mb = 0; // 0 means use default (8 MB)
    ...
    while ((opt = getopt_long(argc, argv, "acd:DEf:Fghi:lmno:p:qSs:t:VvUuyZz123456789", long_options, NULL)) != -1) {
        ...
        case OPTION_BLOCK_CACHE_SIZE: // --block-cache-size
            ...
            if (block_cache_size_mb != 0 && (block_cache_size_mb < 8 || block_cache_size_mb > 1024))
                fwup_errx(EXIT_FAILURE, "--block-cache-size must be 0 (default) or between 8 and 1024 MB");
            break;
        default: /* '?' */
            print_usage();
            fwup_exit(EXIT_FAILURE);
